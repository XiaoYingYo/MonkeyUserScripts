...
unsafeWindow['ChatGPTHeartbeat.user.function']['PostMessage'] = (message) => {
...
async function FindPrimaryBtn(NetworkErrorElement) {
    return new Promise(async (resolve) => {
        ...
        for (let i = 0; i < length; i++) {
            LastMessageParent = $(NetworkErrorElement)
                .parents("div[class^='" + dialogeleclasss[i] + "']")
                .eq(0);
